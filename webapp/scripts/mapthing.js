...
client.open('GET', 'trace.gpx');
client.onreadystatechange = function () {
    if (client.readyState != 4 || client.status != 200) {
        console.error(`Not ready (${client.readyState}) or bad status code (${client.status})`)
        return
    }
    var rawGPX = client.responseText
    gpxParse.parseGpx(rawGPX, function (error, data) {
        if (error != null) {
            throw error
        }
        var speedArr = []
